...
class TimeVaryingWaypoint(sysModel.SysModel):
    ...
    def Reset(self, CurrentSimNanos):
        ...
        if self.waypointModule is None:
            self.bskLogger.bskLog(
                sysModel.BSK_ERROR, 
                "TimeVaryingWaypoint: No waypoint module assigned!"
            )
            return
        ...
        self.bskLogger.bskLog(
            ...
            f"TimeVaryingWaypoint: Reset with initial position {self.startPosition} and velocity {self.startVelocity}"
        )
    ...
    def UpdateState(self, CurrentSimNanos):
        ...
        # Calculate angular velocity based on period
        angular_velocity = 2.0 * math.pi / self.period
        
        # Calculate new position and velocity based on orbit plane
        if self.orbitPlane == 'xy':
            # Circular orbit in XY plane
            x = self.radius * math.cos(angular_velocity * currentTime)
            y = self.radius * math.sin(angular_velocity * currentTime)
            z = 0.0
            
            vx = -self.radius * angular_velocity * math.sin(angular_velocity * currentTime)
            vy = self.radius * angular_velocity * math.cos(angular_velocity * currentTime)
            vz = 0.0
        elif self.orbitPlane == 'yz':
            # Circular orbit in YZ plane
            x = 0.0
            y = self.radius * math.cos(angular_velocity * currentTime)
            z = self.radius * math.sin(angular_velocity * currentTime)
            
            vx = 0.0
            vy = -self.radius * angular_velocity * math.sin(angular_velocity * currentTime)
            vz = self.radius * angular_velocity * math.cos(angular_velocity * currentTime)
...
def run(show_plots):
    ...
    pos_sigma_p = 0.0
    vel_sigma_p = 0.0
    att_sigma_p = 0.0 # 2.0 * math.pi / 180.0
    rate_sigma_p = 0.0 # 0.3 * math.pi / 180.0
...
#    sciencePointGuidance.scTargetInMsg.subscribeTo(sNavObject.transOutMsg)
    ...
    timeVaryingWaypoint = TimeVaryingWaypoint()
    timeVaryingWaypoint.ModelTag = "timeVaryingWaypoint"
